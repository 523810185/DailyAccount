...
#pragma warning disable
    ...
    public static class DragAndDropUtilities
    {
        ...
        public static object DragZone(Rect rect, object value, Type type, bool allowMove, bool allowSwap, int id)
        {
            ...
            if (isMouseOver && (t == EventType.MouseMove || t == EventType.MouseDrag))
            {
            ...
            if (isMouseOver && t == EventType.MouseDown)
            {
                ...
                preventDropAreaRect = new Rect();
                currentDragIsMove = false;
                //Event.current.Use() 不能删除点击事件，不然不能双击响应 - 修改 拖拽 和 双击 的 bug问题
            }
        ...
        {
            var id = GetSelfDragAndDropId(rect);
            return (T)DragZone(rect, value, typeof(T), allowMove, allowSwap, id);
        }
        ...
        /// A draggable zone for both Unity and non-unity objects.
        /// </summary>
        public static object DragZoneList(Rect rect, UnityEngine.Object[] value_array, Type type, bool allowMove, bool allowSwap, int id)
        {
            var tmpNextPreventedDropArea = nextPreventDropAreaRect;
            nextPreventDropAreaRect = new Rect();

            if (value_array == null || value_array.Length <= 0) return null;

            object value = value_array[0] as UnityEngine.Object;
            // Unity null
            if (value == null)
            {
                return null;
            ...
            if (Event.current.type == EventType.MouseDrag && isMouseOver)
            {
                Debug.LogWarning(" EventType.MouseDrag =" + (value as UnityEngine.Object).name);
                ...
                if (unityObject)
                {
                    DragAndDrop.objectReferences = value_array;
                    dragginObjects = value_array;
                }
                else
                {
                    // DragAndDrop.objectReferences = new UnityEngine.Object[] { };
                    ...
                    Debug.LogWarning(" dragginObjects count =" + dragginObjects.Length.ToString());
                }
        ...
        public static object[] DropZoneList(Rect rect, object value, Type type, bool allowSceneObjects, int id)
        {
            ...
            if (rect.Contains(Event.current.mousePosition))
            {
                ...
                if (t == EventType.DragUpdated || t == EventType.DragPerform)
                {
                    ...
                    if (obj_array == null) obj_array = dragginObjects.Where(x => x != null).ToArray();
                    if (obj_array == null) obj_array = DragAndDrop.objectReferences.Where(x => x != null).ToArray();

                    if (type.InheritsFrom<Component>() || type.IsInterface)
                    {
                        if (obj_array == null) obj_array = dragginObjects.OfType<GameObject>().Where(x => x != null).Select(x => x.GetComponent(type)).Where(x => x != null).ToArray();
                        if (obj_array == null) obj_array = DragAndDrop.objectReferences.OfType<GameObject>().Where(x => x != null).Select(x => x.GetComponent(type)).Where(x => x != null).ToArray();
                    }

                    bool acceptsDrag = obj_array != null;

                    if (acceptsDrag && allowSceneObjects == false)
                    {
                        var uObj = obj_array[0] as UnityEngine.Object;
                        if (uObj != null)
                        {
                            if (typeof(Component).IsAssignableFrom(uObj.GetType()))
                            {
        ...
        public static int GetSelfDragAndDropId(Rect rect)
        {
            if (Event.current.rawType != prevEventType && Event.current.rawType != EventType.Used)
            {
                prevEventType = Event.current.rawType;
